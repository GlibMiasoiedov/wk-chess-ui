...
export function fenStringToPositionObject(
  ...
  if (fen === 'start') {
    return fenStringToPositionObject(
...
function fenToPieceCode(piece: FenPieceString) {
  ...
  if (piece.toLowerCase() === piece) {
    code = 'b' + piece.toUpperCase();
  } else {
    // upper case is white piece
    code = 'w' + piece.toUpperCase();
  }

  // DEBUG: Catch the anomalous keys
  if (['bS', 'bT', 'bA', 'wS', 'wT', 'wA'].includes(code)) {
    console.error(`[CRITICAL] Data Corruption Detected in fenToPieceCode! Input: "${piece}" -> Output: "${code}"`);
  }
  return code;
}
